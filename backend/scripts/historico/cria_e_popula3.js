...
SQL().then(SQL => {
  ...
  db.exec(`CREATE TABLE PedidoInfo (id TEXT PRIMARY KEY, texto TEXT, preco REAL, data_pedido DATETIME DEFAULT (datetime('now')), data_entrega DATETIME, estado TEXT DEFAULT 'EM_PRODUCAO', clienteId TEXT, vendedorId TEXT, criado_em DATETIME DEFAULT (datetime('now')), atualizado_em DATETIME DEFAULT (datetime('now')))`);
